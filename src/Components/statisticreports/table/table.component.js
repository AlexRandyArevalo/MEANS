                    ...
                    Body={
                        this.state.dataTable.map((emp, index) =>
                            <tr key={index}>
                                ...
                                <td className="text-truncate td-mw-12 text-muted text-capitalize" title={emp.name}>{emp.name}</td>
                                <td className="text-truncate td-mw-12 text-muted text-capitalize" title={emp.workflow}>{emp.workflow}</td>
                                <td className="text-truncate td-mw-12 text-muted text-capitalize" title={emp.addr}>{emp.addr}</td>
                                <td className="text-truncate td-mw-12 text-muted" title={emp.contact}>{emp.contact}</td>
                                <td className="text-truncate td-mw-12 text-muted text-capitalize" title={emp.respondTag}>{emp.respondTag}</td>
                                <td className="text-truncate td-mw-12 text-muted" title={emp.riskLevel}>{emp.riskLevel}</td>
                                <td className="text-truncate td-mw-12 text-muted text-capitalize" title={emp.needEvac}>{emp.needEvac}</td>
                            </tr>
                        )
                    }
                />
